...
export function resourceActionCreators (baseUrl, resourceName, definition, options = {}) {
  ...
  const { idAttribute, url, urlRoot, rootParam } = definition
  ...
  const fn = (memo, action) => {
    ...
    function getResourceUrl (data = {}) {
      if (urlRoot) {
        const part = (typeof urlRoot === 'function' && urlRoot.call(definition, { data, method })) || urlRoot
      ...
      const part = (typeof url === 'function' && url.call(definition, { data, method })) || url
      return urlAppend(baseUrl, part)
    }
